...
if "`c(username)'"=="gabriellombomoreno" {
	...
	*global presim       "${path}/01_data/2_pre_sim/MRT"
	...
	global proj_3		"GMB_2020_Sub_NoSubs"  // no subsidies and standard exemptions 
	global proj_4		"GMB_2020_Sub_NoExemSubs" // no subsidies and no exemption
}
...
*/	
	...
	global xls_out    	"${path}/03_Tool/Figures_Sub_Sim_GMB.xlsx"
...
mat rownames A = HHPrincipalSource HHPositiveDepan HHSimulationCoverage
...
forvalues scenario = 1/$numscenarios {
	...
	*use "$data_sn/pivot2019.dta", clear
	...
	cap drop depan2 // just for madi 
	duplicates drop hhid codpr, force
	...
	* Output
	...
	save "$tempsim/${elec1_`scenario'}", replace
...
forvalues scenario = 1/$numscenarios {
	*append using `elec1_`scenario''
	append using "$tempsim/${elec1_`scenario'}"
}
