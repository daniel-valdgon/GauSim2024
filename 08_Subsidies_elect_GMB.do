...
use "$presim/05_purchases_hhid_codpr.dta", clear
		...
		*----- HH Coverage
		...
		gen hh_elec_1 = depan > 0 // Option 1: HH uses electricity
		gen hh_elec_2 = use_elec ==1 // Option 2: HH principal source of electricity
...
* Allocate household based on consumption 
	   ...
	   gen domestic = depan1 > 0 |use_elec ==1
...
------------------------------------------------*/
		...
		gen type_client = 1 if domestic == 1
		replace type_client = 2 if domestic == 0
		...
		* Label
		label define type_client 0 "No Elec." 1 "Domestic" 2 "Social" 
...
save "$presim/IO_Matrix_elec.dta", replace
