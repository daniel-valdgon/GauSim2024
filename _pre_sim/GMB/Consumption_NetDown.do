...
merge 1:m codpr using "$presim/IO_percentage.dta", nogen
...
*Indirect effects
vatpush sector_1-sector_70 , exempt(exempted) costpush(cp) shock(TVA) vatable(vatable) gen(TVA_ind_eff_SY)
...
replace TVA=0 if exempted==1 // this should not be needed , temporal fix
...
keep c_inf_mean codpr decile_expenditure depan depan2 depan_for depan_inf hhid hsize informal_purchase achats_net_VAT achat_gross sector achats_net_excise
